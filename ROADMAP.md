...
- **Scoop bucket (Windows)** — goreleaser has native `scoops` support; create a `scoop-bucket` repo under `tsyche`, wire it up in `.goreleaser.yml` similarly to the Homebrew tap. ~20 min.
